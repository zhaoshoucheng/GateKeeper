...
class RoadReportJN extends MY_Controller
{
...
//        }
    ...
    public function queryRoadQuotaData(){
        ...
        $desc="下图利用滴滴数据绘制了该干线全天24小时各项运行指标(停车次数、停车延误、行驶速度、PI)。通过数据分析，该干线的早高峰约为".$mrushTime['s']."~".$mrushTime['e']."，晚高峰约为".$erushTime['s']."~".$erushTime['e']."。与平峰相比，早晚高峰的停车次数达到".round(($avgStopCyclem+$avgStopCyclee)/2,2)."次/车/路口，停车延误接近".round(($avgStopDelaym+$avgStopDelaye)/2,2)."秒/车/路口，行驶速度也达到".round(($avgSpeedm+$avgSpeede)/2,2)."千米/小时左右。";
