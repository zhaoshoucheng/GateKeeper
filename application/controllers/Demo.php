...
class Demo extends MY_Controller
{
    ...
    {
        $params = [
            ["2020-05-06 07:00:00", "2020-05-06 09:00:00"],
            ["2020-05-07 07:00:00", "2020-05-07 09:00:00"],
            ["2020-05-08 07:00:00", "2020-05-08 09:00:00"],
            ["2020-05-09 07:00:00", "2020-05-09 09:00:00"],
            ["2020-05-06 16:30:00", "2020-05-06 18:00:00"],
            ["2020-05-07 16:30:00", "2020-05-07 18:00:00"],
            ["2020-05-08 16:30:00", "2020-05-08 18:00:00"],
            ["2020-05-09 16:30:00", "2020-05-09 18:00:00"],
        ];
        foreach ($params as $param) {
            $sql = 'SELECT count(*) as cnt,avg(delay) as avg_delay,downstream_ramp from cn_signal_pro_freeway_segment_index_online_* where city_id=23 and day_time_hms>="' . $param[0] . '" and day_time_hms<="' . $param[1] . '" group by downstream_ramp order by cnt desc limit 10000';
            ...
            echo "\n";
            echo "\n";
            print_r(json_encode(array_slice($outputList, 0, 30), JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
            echo "\n";
            echo "\n";
        }
        exit;
    ...
    public function districtsTest()
    {
        ...
        foreach ($junctions as $junctionid) {
            echo "INSERT INTO `area_junction_relation` (`id`, `area_id`, `junction_id`, `user_id`, `update_at`, `create_at`, `delete_at`) VALUES (NULL, '768', '" . $junctionid . "', '0', '2020-05-12 21:29:40', '2020-05-12 21:29:40', '1970-01-01 00:00:00');<br/>";
        }
        exit;
