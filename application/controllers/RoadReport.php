...
class RoadReport extends MY_Controller
{
    ...
    public function queryRoadQuotaData(){
        ...
        $this->response(['info'=>['instructions'=>"报告采用综合评估指数（PI）来分析路口整体及各维度交通运行情况XXXX",'desc'=>$desc,'morning_rush_time'=>$mrushTime['s']."~".$mrushTime['e'],"evening_rush_time"=>$erushTime['s']."~".$erushTime['e']],'chart'=>$chartDatas]);
