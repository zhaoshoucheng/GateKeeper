...
class AdaptMovement extends MY_Controller{
    ...
    public function proxy()
    {
        ...
        if($reqMethod == 'GET'){
            ...
            $ret = httpGET($this->config->item('signal_control_interface')."/adaptmovement/".$funcName,$params);

        }elseif ($reqMethod == 'POST'){
            $params = file_get_contents("php://input");
            $ret = httpPOST($this->config->item('signal_control_interface')."/adaptmovement/".$funcName,$params,0,'raw');
        }
        if($ret){
