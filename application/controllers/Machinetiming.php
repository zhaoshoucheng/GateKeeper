...
class Machinetiming extends MY_Controller{
    ...
    public function proxy()
    {
        ...
        if($reqMethod == 'GET'){
            ...
            if(isset($params['read_only']) && $params['read_only']==1){
                $ret = httpGET($this->config->item('signal_control_interface')."/machinetiming/".$funcName,$params);
            }else{
                $ret = httpGET($this->config->item('signal_control_interface')."/machinetiming/".$funcName,$params);
            }
        }elseif ($reqMethod == 'POST'){
            $params = file_get_contents("php://input");
            $ret = httpPOST($this->config->item('signal_control_interface')."/machinetiming/".$funcName,$params,0,'raw');
        }
        if($ret){
