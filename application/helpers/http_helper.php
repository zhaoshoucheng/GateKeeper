...
if (!function_exists('httpGET')) {
    function httpGET($url, $query=array(), $msTimeout = 20000, $headers = array()){
        ...
        com_log_notice('_com_http_success', ["cspanid"=>$cSpanId, "url"=>$url, "args"=>http_build_query($query), "response"=>substr($ret,0,50*1024), "errno"=>$responseCode, 'proc_time'=> $totalTime]);
        return $ret; 
    }
}
...
if (!function_exists('httpPOST')) {
    function httpPOST($url, $data, $msTimeout = 0, $contentType='x-www-form-urlencoded', $headers = array()){
        ...
        com_log_notice('_com_http_success', ["cspanid"=>$cSpanId, "url"=>$url, "args"=>$data, "response"=>substr($ret,0,50*1024), "errno"=>$responseCode, 'proc_time'=> $totalTime]);
        return $ret;
    }
}
