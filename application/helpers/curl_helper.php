 ...
 * @param mixed $options 传入的curl_opt字段。 其中默认调用方法为post CURLOPT_POST=1, 将请求的数据直接返回CURLOPT_RETURNTRANSFER=1, 超时时间为5秒 CURLOPT_TIMEOUT=5, 发起连接前等待的时间1秒 CURLOPT_CONNECTTIMEOUT=1
 * @param $caller 调用方
 * @return array
...
function curl($url, $params=array(), $options=array(), $retries = 2, $connectTime = 1, $caller = '' , $aExtInfo = array()){ // retries 重试次数, 默认为2
    if ($caller) {
        ...
        if ( ! isset($defaultOpt[CURLOPT_HTTPHEADER])) {
            $defaultOpt[CURLOPT_HTTPHEADER] = array();
        }
        ...
        $fields = array("url", "total_time", "namelookup_time", "connect_time", "pretransfer_time", "starttransfer_time", "redirect_time");
        foreach ($fields as $v) {
            if ($isCliReq) {
        ...
        if ($errno != 0) { //如果超时则打印出来
            if (function_exists('com_log_warning')) {
                com_log_warning("_com_http_failure", $errno, $errmsg, array(
                    "args"            => $jsonParams,
...
function curlNoLog($url, $params=array(), $options=array(), $retries = 2, $connectTime = 1){ // retries 重试次数, 默认为2
    $curlInstance=  curl_init($url);
    $defaultOpt=array(
        CURLOPT_POST=>1,
        CURLOPT_RETURNTRANSFER=>1,
    ...
    while($count--){
        ...
        if ($ret !== FALSE){
            ...
            if (function_exists('com_log_warning')) {
                com_log_warning('_com_http_failure', $errno, $errmsg, array("cspanid"=>$spanId, "message"=> "request fail", "url"=>$url, "args"=>$jsonParams, "result"=>$ret, "retry"=>$retries));
            }
        }
    }
    ...
    if($errno != 0){ //如果超时则打印出来
        ...
        if(function_exists('com_log_warning')) {
            com_log_warning("_com_http_failure", $errno, $errmsg, array("cspanid"=>$spanId, "args"=>$jsonParams));
        }
    }else{
        if (function_exists('com_log_notice')) {
            $time_elapsed_arr = array();
            ...
            com_log_notice('_com_http_success', array_merge(array("cspanid"=>$spanId, "url"=>$url, "args"=>$jsonParams, "response"=>$ret, "errno"=>$errno, "errmsg"=>$errmsg, 'proc_time'=>$info['total_time']), $time_elapsed_arr));
        }
    }
    curl_close($curlInstance);
 ...
 * @date   2015-03-06
 */
function curlMultiProcess($urls = array() , $params=array(), $options=array(), $retries = 2, $connectTime = 1) {
    $mh = curl_multi_init();
    $arr_spanid = array();
    $defaultOpt=array(
        CURLOPT_POST=>1,
    ...
    foreach($conn as $key => $handle) {
        ...
        if(function_exists("com_log_notice")){
            com_log_notice('_com_http_success', array(
