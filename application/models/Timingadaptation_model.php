...
class Timingadaptation_model extends CI_Model
{
    ...
    private function formatAdaptionTimingInfo($adapt, $current, $params)
    {
        ...
        foreach ($adapt['tod'] as $tk => $tod) {
            ...
            $adapt['tod'][$tk]['extra_time']['tod_end_time']
                = date('H:i', strtotime($adapt['tod'][$tk]['extra_time']['tod_end_time']));
            $adapt['tod'][$tk]['extra_time']['tod_start_time']
                = date('H:i', strtotime($adapt['tod'][$tk]['extra_time']['tod_start_time']));
            ...
            foreach ($tod['movement_timing'] as $mk => $movement) {
                ...
                $adapt['tod'][$tk]['movement_timing'][$mk]['flow']['twice_stop_rate']
                    = $flows[$adapt['tod'][$tk]['movement_timing'][$mk]['flow']['logic_flow_id']] ?? '/';
                ...
                // 自适应和基准配时数据递归合并
                $greens = arrayMergeRecursive($adapt['tod'][$tk]['movement_timing'][$mk]['timing'], $greenCurrents);

                // 数据处理
                $adapt['tod'][$tk]['movement_timing'][$mk]['timing'] = array_map($formatTimingCallback, $greens);

                //提取 flow id
                $adapt['tod'][$tk]['movement_timing'][$mk]['logic_flow_id']
                    = ($adapt['tod'][$tk]['movement_timing'][$mk]['flow']['logic_flow_id'] ?? '')
                    . ($adapt['tod'][$tk]['movement_timing'][$mk]['flow']['comment'] ?? '');
            }

            // 移除 flow id 为空的元素
            $adapt['tod'][$tk]['movement_timing'] = call_user_func($removeEmptyFlowIdItemCallback, $adapt['tod'][$tk]['movement_timing']);

            $adapt['tod'][$tk]['movement_timing'] = Collection::make($adapt['tod'][$tk]['movement_timing'])
                ->groupBy('logic_flow_id', $mergeSameFlowIdTimingCallback)
                ->values()->get();

            unset($adapt['tod'][$tk]['stage']);
        }
