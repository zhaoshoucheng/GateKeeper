    ...
    */
    public function getSpaceTimeDiagram($data)
    {
        ...
        foreach ($data['junctions'] as $k=>$v) {
            $tempCycleOffset[$v['junction_id']]['cycle'] = intval($v['cycle']) ?? 0;
            $tempCycleOffset[$v['junction_id']]['offset'] = intval($v['offset']) ?? 0;
            ...
            if (!empty($v['reverse_traj'])) {
                foreach ($v['reverse_traj'] as $kk=>$vv) {
                    foreach ($vv as $kkk=>$vvv) {
                        $vvv = (array)$vvv;
                        // 秒数 % cycle
                        $result['dataList'][$k]['reverse_traj'][$kk][$kkk][0] = $vvv['timestamp'] % $cycleArr[$v['juction_id']];
                        // 值
