...
class TraceLog
{
    ...
    private $useGlobalTraceId = 1;//是否使用全局trace_id

    function __construct($name = "default", $path)
    {
    ...
    public static function getInstance($name, $path)
    {
        if (isset(self::$loggerMap[$name])) {
            return self::$loggerMap[$name];
        }
        return new TraceLog($name, $path);
    }
    ...
    private function t_get_span_id()
    {
        ...
        $this->_global_span_id = t_gen_random_id();
        return $this->_global_span_id;
    }
