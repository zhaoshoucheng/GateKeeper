    ...
    public function call($method, $args)
    {
        ...
        while ($retry) {
            try {
                ...
                com_log_warning('_com_thrift_failure', 0, $e->getMessage(), array("cspanid"=>$spanId, "config"=> json_encode($this->config), 'method' => $method, "args"=> json_encode($args), 'e' => $e->getMessage(), 'retry' => $retry));
                $retry--;
                $this->close();
